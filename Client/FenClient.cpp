...
void FenClient::on_boutonEnvoyer_clicked()
{
    ...
    messageAEnvoyer.SetMessage(tr("<strong>") + pseudo->text() +tr("</strong> : ") + message->text());
    messageAEnvoyer.SetSourceName(pseudo->text());
    messageAEnvoyer.SetDestinataireName("tous");
    messageAEnvoyer.SetDestinataireName("tous");

    stream << (quint16) 0;
    messageAEnvoyer.serialize(stream);
    stream.device()->seek(0);
    stream << (quint16) (buffer.size() - sizeof(quint16));

    socket->write(buffer); // On envoie le paquet
